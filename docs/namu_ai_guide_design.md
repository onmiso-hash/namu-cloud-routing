...
| `src/ui.py` | 말풍선 조각(`ask_widget()`) 추가, `page()`에서 부름 | 고침 |
| `src/web_auth.py` | `/auth/ask` (POST) 라우트 한 줄 추가 | 고침 |
...
| 6 | 말풍선 화면 + **고지 한 줄** | `ui.py` | 브라우저에서 눌러 본다 |
| 7 | **안전 화면에 고지 문단** | `pages.py` | `/safety`에 문단이 생긴다 |
