...
NAMU_ASK_API_KEY=<열쇠>            # 이름에 회사가 들어가지 않는다 — 아래
```
...
**열쇠 이름에도 회사를 넣지 않는다** (2026-08-08 사용자 지적으로 고침). 앞 판은
`GEMINI_API_KEY`였다 — 회사와 모델은 갈아 끼울 수 있게 빼 놓고 열쇠 칸에만 회사
이름을 박아 둔 것이라 결이 어긋났다. 그대로 두면 회사를 바꾸는 날 배포 설정에서
이름 하나를 더 바꿔야 하고, 옛 이름이 남아 있어도 아무 신호가 안 뜬다.
