...
_SYNONYMS = {
    ...
    "가격": ("무료",),
    "비용": ("무료",),
    # 2026-08-15에 '자주 묻는 질문'을 '돈이 드나요?'에서 '무료인가요?'로 고쳐
    # 쓰면서 '돈'이라는 낱말이 문서에서 사라졌다. 방문자는 여전히 '돈이
    # 드나요'라고 묻는다.
    #
    # **조사가 붙은 채로 적는 이유** — 위의 '요금·가격·비용'과 달리 '돈'은 한
    # 글자다. 꼬리를 떼며 찾아 주는 장치(_forms)는 두 글자에서 멈추므로 '돈'에는
    # 닿지 못하고, 낱말 자체도 두 글자가 안 되면 아예 걸러진다(_terms). 그래서
    # 이 낱말만은 조사가 붙은 형태로 적는다.
    #
    # **여기 없으면 답을 손에 쥐고도 못 낸다** — 검색이 0건이면 `Ask`는 자료를
    # 꺼내 보기 전에 '모르겠다'로 끝난다(ask.py의 `if not chunks`). 값에 대한
    # 답은 화면 '자주 묻는 질문'에 있고 그 글은 `homepage_text()`로 늘 프롬프트
    # [0]번에 실리는데, 그 문턱을 못 넘으면 [0]번을 볼 기회가 없다.
    "돈이": ("무료",),
    "돈은": ("무료",),
    "돈을": ("무료",),
    "돈도": ("무료",),
