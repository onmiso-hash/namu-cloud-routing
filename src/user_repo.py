...
def _promisor_url_args(url: str) -> list[str]:
    ...
    명령마다 ①주소 없이 죽는가 ②얹으면 첨부가 딸려 내려오는가를 따로 재서 정했다.
    실서버 사본과 같은 상태(기억 파일 몸통이 오브젝트 저장소에서 사라지고 디스크
    파일만 남은 사본)를 만들어 놓고 진짜 git으로 잰 결과다.
    ...
    그래서 `status`와 `add -A`에는 **얹지 않는다 — 필요가 없기 때문이다.** 주소
    없이도 정상으로 돌았다. 위험해서 안 얹는 것이 아니다: 이 둘에 주소를 얹어도
    첨부는 안 내려왔고 사본 크기도 그대로였다(표의 3열·4열). 안 써도 되는 것을
    굳이 쓰지 않는다는 뜻일 뿐이다.
