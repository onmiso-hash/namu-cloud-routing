...
def _fork() -> str:
    ...
    return (
        ...
        "<h4>터미널에서 AI를 쓴다</h4>"
        "<p>Claude Code·Antigravity CLI(agy)·Grok을 쓰신다면 MCP 주소를 붙이는 "
...
def start_page(logged_in: bool = False) -> str:
    body = (
        ...
        + ui.notice(
            ...
            "CLI(agy)·Grok을 사용하면서 함께 나무를 사용하시겠다면 "
...
def faq_page(logged_in: bool = False) -> str:
    body = (
        ...
        + ui.faq(
            [
                # 질문을 '돈이 드나요?'에서 바꾼 이유(2026-08-15, 사용자 결정):
                # 돈을 앞세우면 아직 값을 매기지도 않은 서비스에 먼저 경계가
                # 선다. 지금 나무는 소수의 분들과 함께 써 보며 의견을 듣는
                # 단계라 요금을 말할 자리가 아니고, 그래서 답이 먼저 보이는
                # 질문으로 바꿨다. 앞으로의 요금제를 여기서 이야기하지 않는
                # 것도 같은 이유다.
                (
                    "무료인가요?",
                    "<p><b>네, 무료입니다.</b> 나무 클라우드 서비스는 지금 소수 "
                    "분들과 함께 써 보며 의견을 듣고 개선하는 단계입니다. 이후 "
                    "서비스에 대한 유료화 논의는 진행된 바 없으며, 서버 "
                    "시스템이 허용하는 범위에서 무료로 제공됩니다.</p>"
                    "<p>결제 화면도, 카드 정보를 넣는 칸도 없습니다. "
                    "사용하시다 불편한 점을 알려 주시는 것이 지금 나무에는 "
                    "가장 큰 도움이 됩니다.</p>",
                ),
                (
                    "저장소가 뭔가요?",
                ...
                (
                    "Claude Code·Antigravity CLI(agy)·Grok을 쓰는데요?",
    ...
    return ui.page(
        ...
        current="/faq",
        cta="me" if logged_in else "start",
        # 검색 결과에 뜨는 한 줄. 첫 낱말이 '요금'이면 문답을 바꾼 뜻이 없다.
        description="무료·저장소·여러 AI 연결·그만두는 법 등 나무 클라우드에 "
        "대해 자주 묻는 질문.",
    )
