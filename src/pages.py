...
**문구의 기준일: 2026-08-01.** 옛 안내서(`namu-agent/docs/*.html`)를 옮겨 오지
않았다. 그 문서들에는 폐기된 설명이 남아 있다 — 전원 공용 열쇠 + `?user=`
방식(namu-59에서 사라짐), "그릇은 둘, 곧 셋"(지금 넷). 여기 적힌 것은 전부
...
def _fork() -> str:
    ...
    터미널 사용자를 이 길로 흘려보내면 반쪽짜리 나무를 쓰게 된다 — 이 주소로는
    기억 도구 세 개만 넘어가고, 세션 브리핑·작업 절차·마무리 점검은 플러그인에만
    있다. 게다가 기억이 쌓이는 자리도 둘로 갈린다.
    """
    return (
        '<div class="fork">'
        ...
        "<b>플러그인으로 설치</b>하셔야 합니다. 이 주소로는 기억 도구 세 개만 "
...
def start_page(logged_in: bool = False) -> str:
    body = (
        ...
        + "<h2>붙이고 나면</h2>"
        "<p>대화 중에 AI가 세 가지를 할 수 있게 됩니다 — <b>기억 꺼내기</b>, "
        "<b>기억 남기기</b>, <b>기억 찾기</b>. 처음에는 "
        "\"지금까지 내 기억 꺼내 봐\"처럼 말로 시켜 보시면 됩니다.</p>"
...
def memory_page(logged_in: bool = False) -> str:
    body = (
        ...
        "</div>"
        "<h2>여기에 담기지 않는 것</h2>"
        "<p><b>작업일지</b>는 이 주소로 남길 수 없습니다. 작업 기록은 회원님 "
        "PC에서 도는 나무가 그 PC 기준으로 남기는 것이라, 웹 AI가 끼어들면 "
        "기록이 두 갈래로 갈라집니다. 대신 PC의 나무가 올려 둔 작업일지를 "
        "<b>읽는 것</b>은 됩니다 — 내 기억 화면의 작업일지 칸에서 보실 수 "
        "있습니다.</p>"
        "<h2>지금은 이렇게 동작합니다</h2>"
        "<ul>"
        "<li><b>기억 꺼내기</b>는 세 그릇을 한꺼번에 훑어 최근 것을 "
        "돌려줍니다.</li>"
        "<li><b>기억 찾기</b>는 아직 <b>교훈</b> 그릇만 낱말로 찾습니다. "
        "나머지 그릇도 저장·열람은 되므로 기록이 사라지지는 않습니다.</li>"
...
def faq_page(logged_in: bool = False) -> str:
    body = (
        ...
        + ui.faq(
            [
                ...
                (
                    ...
                    "<p>그 경우에는 이 주소를 붙이지 마세요. 주소로는 기억 도구 "
                    "세 개만 넘어가고, 세션 브리핑·작업 절차·마무리 점검처럼 "
                    "나무의 나머지 절반이 따라오지 않습니다. 게다가 기억이 쌓이는 "
                    "자리도 둘로 갈라집니다.</p>"
        ...
        "<h4>아직 안 되는 것</h4>"
        "<p class='muted' style='margin:0'>기억 찾기는 아직 <b>교훈</b> 그릇만 "
        "낱말로 찾습니다. 웹에서 기억을 고쳐 쓰는 것도 아직 안 됩니다"
        "(쪽지 떼기만 예외입니다). 접속 주소는 사람이 브라우저로 받아 손으로 "
        "붙이는 방식입니다.</p>"
        "</div>"
    )
    return ui.page(
