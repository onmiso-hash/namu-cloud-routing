...
def memory_page(logged_in: bool = False) -> str:
    body = (
        ...
        "합니다. PC에서 실행되는 나무는 지금 열어 둔 폴더가 곧 프로젝트지만, "
        ...
        "띄어 쓴 낱말이 <b>전부 들어 있는 것</b>만 걸리고, 순서는 상관없습니다.</li>"
...
def safety_page(logged_in: bool = False) -> str:
    body = (
        ...
        # 말풍선의 한 줄(ui.ASK_NOTICE)이 좁아서 다 못 담는 내용을 여기 편다 —
        # 그 줄의 '자세히'가 오는 곳이다(설계서 10-1절). 말풍선을 켜 둔 채 이
        # 문단을 빼면, 우리는 방문자에게 알리지 않고 글을 넘기는 셈이 된다.
        + '<h2 id="ai">화면 구석의 안내원에게 물으면 그 글은 밖으로 나갑니다</h2>'
        + "<p>오른쪽 아래 말풍선에 적은 질문은 <b>Google의 AI</b>로 보내집니다. "
        "나무가 <b>무료 등급</b>을 쓰고 있어서, Google은 그 약관에 따라 "
        "<b>보낸 글과 받은 답을 자기 제품을 만드는 데 쓸 수 있고 사람이 직접 읽고 "
        "표시를 달 수도 있습니다.</b> 그래서 말풍선에는 "
        "<b>개인정보나 접속 주소·열쇠를 적지 마세요.</b></p>"
        "<p>나가는 것은 <b>방문자가 직접 친 글뿐입니다.</b> 안내원이 답할 때 보는 "
        "자료는 이 홈페이지의 화면 글과 펴낸 안내서뿐이고, <b>회원님의 기억은 "
        "볼 길 자체가 없습니다</b> — 안내원이 읽는 자료에 담기지 않고, 기억을 읽는 "
        "기능을 부르지도 않습니다. 주고받은 대화는 <b>어디에도 저장하지 않습니다.</b> "
        "창을 닫으면 사라집니다.</p>"
        + ui.notice(
            "무료 등급을 고른 것은 값 때문만은 아닙니다. 다만 이 고지를 달지 않고 "
            "쓰는 쪽은 고르지 않았습니다 — 나중에 유료 등급으로 올려 이 문단이 "
            "필요 없어지면, 그때 이 자리의 글도 함께 고칩니다.",
            tone="info",
        )
        ...
        + ui.faq(
            [
                ...
                (
                    ...
                    'target="_blank" rel="noopener">직접 서버 운영하기 안내서 ↗</a>'
                    "</p>",
                ),
                (
                    "그만두려면 어떻게 하나요?",
