...
_DEFAULT_PER_PERSON = 100
_DEFAULT_DAILY_CAP = 400
"""사이트 전체 하루 한도(설계서 7-1절).

Google AI Studio 한도 화면에서 실제 계정의 `gemini-3.5-flash-lite` 줄을 확인한
값이다(2026-08-08, 회원이 화면을 캡처해 보여줬다): 분당 15회, **하루 500회**.
전에는 이 값을 몰라 Flash 계열 전체를 묶은 250~1,000 범위로 추정했었다.

**하루 500회의 80%인 400을 사이트 전체 한도로, 그중 한 사람 몫은 100으로
잡는다**(회원이 정한 값, 2026-08-08). 400으로 잡는 이유는 이전과 같다 — 실제
한도보다 낮게 잡아야 방문자가 딱딱한 오류 대신 부드러운 문구를 먼저 본다.
숫자가 다시 바뀌면 `NAMU_ASK_DAILY_CAP`·`NAMU_ASK_PER_PERSON`으로 올린다
(다시 빌드하지 않는다).
"""
