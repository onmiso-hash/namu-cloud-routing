...
def count_users(conn: sqlite3.Connection) -> int:
    """지금까지 GitHub으로 연결한 사람의 수 — 홈페이지가 보여 주는 그 숫자.

    **왜 이 장부가 셀 자료인가.** 이 표에는 `/auth/github/callback`이 GitHub의
    답을 받아 `upsert_user`를 부를 때만 줄이 생긴다. 즉 한 줄 = 실제로 존재하는
    GitHub 계정 하나가 이 서비스에 로그인한 사실이고, github_id가 UNIQUE라
    같은 사람이 열 번 들어와도 한 줄이다.

    **시험용을 어떻게 가리나 — 가릴 필요가 없다.** 사용자 데이터가 쌓이는
    폴더(`NAMU_STORE_ROOT/users/`)에는 신원 계층이 생기기 전(2026-07-18~19)에
    손으로 만든 시험용 칸이 남아 있다(`onmiso`·`otheruser`·`verifytest`).
    그 셋은 이 장부에 줄이 없다 — 로그인을 거쳐 생긴 것이 아니기 때문이다.
    그래서 **폴더를 세지 않고 장부를 세는 것 자체가 시험용을 거르는 기준**이다.
    폴더 개수를 세면 4가 나오고 장부를 세면 2가 나오는데, 뒤쪽이 맞는 수다
    (2026-09-04 실측).

    **접속 주소를 폐기한 사람도 센다**(`mcp_revoked_at`). 폐기는 "AI에 붙인
    주소를 끊었다"는 뜻이지 탈퇴가 아니고, 다시 로그인하면 같은 자리로 돌아온다.
    "지금 붙어 있는 사람 수"를 따로 보이고 싶어지면 그때 함수를 하나 더 만든다 —
    한 함수가 조건에 따라 다른 것을 세면 화면마다 다른 뜻이 붙는다.

    조회만 한다. 이 숫자는 로그인 없이 누구나 보는 자리(홈페이지)로 나가므로,
    **누가 가입했는지는 절대 함께 내보내지 않는다** — 세는 것과 명단은 다르다.
    """
